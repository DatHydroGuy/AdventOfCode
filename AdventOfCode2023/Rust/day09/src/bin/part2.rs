...
fn part2(input: &str) -> String {
    ...
    for init_vec in result {
        ...
        let answer = backtrack_difference_pyramid_start(diffs);
        results.push(answer);
    }
...
fn backtrack_difference_pyramid_start(mut input: Vec<Vec<i32>>) -> i32 {
    input.reverse();
    let mut result = 0;
    input.iter().for_each(|x| {
        result = x.first().unwrap() - result;
    });
    result
}
...
10 13 16 21 30 45",
    ...
    #[test]
    fn test_backtrack_difference_pyramid_start1() {
        let expected = -3;
        let result = backtrack_difference_pyramid_start(vec![
            vec![0, 3, 6, 9, 12, 15],
            vec![3, 3, 3, 3, 3],
            vec![0, 0, 0, 0],
        ]);
    ...
    #[test]
    fn test_backtrack_difference_pyramid_start2() {
        let expected = 5;
        let result = backtrack_difference_pyramid_start(vec![
