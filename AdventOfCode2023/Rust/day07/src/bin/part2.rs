...
fn rank_hand(hand: Vec<&str>) -> (u64, Vec<u8>, &str, u64) {
    const CARD_POSITIONS: &str = "1J23456789T0QKA";

    let cards = hand[0];
    let mut hand_data = cards.chars().fold(HashMap::new(), |mut acc, chr| {
        *acc.entry(chr).or_insert(0) += 1;
        acc
    });

    let num_of_jokers = *hand_data.get(&'J').unwrap_or(&0);
    *hand_data.get_mut(&'J').unwrap_or(&mut 0) = 0;
    ...
    for (i, hand_group_count) in hand_group_counts.iter().enumerate() {
        if i == 0 {
            score += num_of_jokers as u64 * 10u64.pow((4 - i) as u32);
        }
...
mod tests {
    ...
    #[test]
    fn test_rank_hand2() {
        let expected = (41000, vec![10, 5, 5, 1, 5], "T55J5", 684);
    ...
    #[test]
    fn test_rank_hand4() {
        let expected = (41000, vec![13, 10, 1, 1, 10], "KTJJT", 220);
    ...
    #[test]
    fn test_rank_hand5() {
        let expected = (41000, vec![12, 12, 12, 1, 14], "QQQJA", 483);
    ...
    fn test_rank_hands() {
        let expected = vec![
            ...
            (41000, vec![10, 5, 5, 1, 5], "T55J5", 684),
            (41000, vec![12, 12, 12, 1, 14], "QQQJA", 483),
            (41000, vec![13, 10, 1, 1, 10], "KTJJT", 220),
        ];
    ...
    #[test]
    fn test_part2() {
        let expected = String::from("5905"); //765 * 1 + 28 * 2 + 684 * 3 + 483 * 4 + 220 * 5)
        let result = part2(
            "32T3K 765
T55J5 684
KK677 28
